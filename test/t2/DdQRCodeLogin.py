...
class DdQRCodeLogin(QRCodeLogin):
    ...
    def full_login(self, proxy_key=None, proxy_area="", proxy_isp=0) -> dict:
        """
        完整登录流程：展示二维码 → 扫码 → 短信验证（如有）→ 返回 cookies
        使用单一 session 保持整个请求链路的 cookies

        :param proxy_key: 青果代理key，传入则启用代理（避免IP异常触发短信验证）
        :param proxy_area: 代理地区代码，如 "350500"
        :param proxy_isp: 代理运营商 0=不筛选 1=电信 2=移动 3=联通
        """
        import uuid
        session = requests.Session()
        session.headers.update(self.headers)
        ...
        if proxy_mgr:
            logger.info(f"[代理] 当前代理剩余: {proxy_mgr.remaining_seconds():.0f}秒，请尽快扫码！")
        ...
        while True:
            # 代理续期检查：如果剩余时间不足，自动刷新代理
            # 注意：刷新代理意味着IP会变，可能导致服务端认为IP异常
            # 但总比代理过期连接失败强
            if proxy_mgr:
                remaining = proxy_mgr.remaining_seconds()
                if remaining <= 0:
                    logger.warning(f"[代理] 代理已过期，刷新新代理...")
                    try:
                        proxy_mgr.refresh_if_needed(session, min_remaining=5)
                    except Exception as e:
                        logger.error(f"[代理] 刷新代理失败: {e}")
                elif remaining <= 15:
                    logger.warning(f"[代理] 代理即将过期（剩余{remaining:.0f}秒），请加快扫码！")
    ...
    def _do_sms_verify(self, session: requests.Session, token: str, verify_ticket: str, web_did: str = "") -> dict:
        ...
        for round_num in range(1, 6):
            ...
            for poll in range(1, 40):
                ...
                # 代理过期时自动刷新（通过 session.proxies 检查）
                if session.proxies:
                    try:
                        poll_data_with_ticket = {**base_data, "verify_ticket": poll_ticket}
                        poll_resp = session.post(check_url, data=poll_data_with_ticket)
                    except Exception as proxy_err:
                        logger.warning(f"[轮询第{poll}次] 代理连接失败: {proxy_err}，清除代理使用直连...")
    ...
    def resume_sms_login(self, code: str, state_file: str = "/tmp/dd_login_state.json") -> dict:
        """
        从保存的状态文件恢复，直接提交验证码完成登录。
        用于 full_login() 发完短信后因非交互环境中断时续跑。
        """
        import json
        ...
        token = state["token"]
        verify_ticket = state["verify_ticket"]
        saved_cookies = state["cookies"]
        csrf_token = state["csrf_token"]

        logger.info(f"[resume] token: {token}")
        logger.info(f"[resume] verify_ticket: {verify_ticket}")
        logger.info(f"[resume] 恢复 cookies: {saved_cookies}")

        # 重建 session 并注入保存的 cookies
        session = requests.Session()
        session.headers.update(self.headers)
        session.headers.pop('Accept-Encoding', None)
        session.headers["x-tt-passport-csrf-token"] = csrf_token
        for k, v in saved_cookies.items():
            session.cookies.set(k, v, domain=".jinritemai.com")

        # 直接从 Step 5 开始：提交验证码
        logger.info(f"[resume] 提交验证码: {code}")
        check_resp = session.post(
            "https://doudian-sso.jinritemai.com/passport/web/mobile/check_code/",
        ...
        logger.info(f"[check_code] 响应: {check_resp.text}")
        logger.info(f"[check_code] Set-Cookie: {dict(check_resp.cookies)}")
        assert check_resp.status_code == 200, f"验证码提交失败: {check_resp.text}"
        check_res = check_resp.json()
        assert check_res.get("message") == "success", f"验证码校验失败: {check_resp.text}"
        new_ticket = check_res.get("data", {}).get("ticket")
        logger.info(f"[check_code] 验证码通过，new_ticket: {new_ticket}")

        # Step 6: 带新 ticket 完成 check_qrconnect，最多循环3轮处理可选二次验证
        current_ticket = new_ticket
        for round_num in range(1, 4):
            logger.info(f"[check_qrconnect] 第{round_num}轮，ticket: {current_ticket}")
            ...
            assert final_resp.status_code == 200, f"最终验证异常: {final_resp.status_code}"
            final_data = final_resp.json()
            logger.info(f"[check_qrconnect 第{round_num}轮] 响应: {final_resp.text}")

            status = final_data.get('data', {}).get('status')
            if status == "3":
                # 登录成功
                redirect_url = final_data['data']['redirect_url']
                session.get(redirect_url, allow_redirects=True)
                all_cookies = session.cookies.get_dict()
                logger.info(f"[resume] 登录成功，获取到 {len(all_cookies)} 个 cookies")
                return all_cookies

            # 服务端返回新一轮可选验证（is_optional_verify=true），直接用新 ticket 继续
            # verify_ticket 和 is_optional_verify 在响应顶层，不在 data 里
            next_ticket = final_data.get('verify_ticket')
            is_optional = final_data.get('is_optional_verify', False)
            if next_ticket and is_optional:
                logger.info(f"[check_qrconnect] 收到可选验证 ticket，跳过，继续下一轮")
                current_ticket = next_ticket
                continue

            raise AssertionError(f"check_qrconnect 第{round_num}轮未成功: {final_resp.text}")

        raise AssertionError("check_qrconnect 超过最大轮次仍未登录成功")
